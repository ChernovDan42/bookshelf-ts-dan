...
const GerbIcon = () => {
  return (
    <SvgIcon className={style.gerb} sx={{ width: 'auto', height: 'auto' }}>
      <svg
...
export const Donate = () => {
  ...
  return (
    <Box className={clsx(style.donateContainer, style.gradient)}>
      <Box sx={{ maxWidth, padding: '24px 32px', margin: 'auto' }}>
        <Box className={style.donateHeader}>
          <Typography
            color="#FFF"
            sx={{
              fontWeight: '700',
        ...
        <List className={style.donateList}>
          {donateFunds.map((fund, index) => {
            const foundImage = isRetina ? fund.retinaImg : fund.img;
            return (
              <ListItem key={fund.title} className={style.listItem}>
                <Link
                  href={fund.url}
                  target="_blank"
                  className={style.donateItemLink}
                  rel="noopener noreferrer nofollow"
                  aria-label={fund.title}
                >
                  <img
                    className={style.donateImg}
                    src={foundImage}
                    alt={fund.title}
                    loading="lazy"
                  />
